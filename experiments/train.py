"""
Rediscover experiment training script. Single-GPU, single-file.
Adapted from miolini/autoresearch-macos (MPS-compatible fork of Karpathy's autoresearch).

This is the file that research agents modify. prepare.py and its evaluate_bpb are fixed.

Usage: uv run experiments/train.py
"""
...
# Add experiments/ to path so prepare.py is importable
...
class GPT(nn.Module):
    ...
    def setup_optimizer(
        self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02, weight_decay=0.0, adam_betas=(0.8, 0.95), scalar_lr=0.5
    ):
        ...
        for shape in sorted({p.shape for p in matrix_params}):
            ...
            param_groups.append(
                dict(kind="muon", params=group_params, lr=matrix_lr, momentum=0.95, ns_steps=5, beta2=0.95, weight_decay=weight_decay)
            )
...
# ---------------------------------------------------------------------------
# Optimizer (MuonAdamW, single GPU only — MPS compatible)
# ---------------------------------------------------------------------------
...
# Detect device early for conditional compilation
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def __init__(self, param_groups):
        ...
        if device_type in ("cuda", "cpu"):
            self.adamw_step_fn = torch.compile(adamw_step_fused, dynamic=False, fullgraph=True)
            self.muon_step_fn = torch.compile(muon_step_fused, dynamic=False, fullgraph=True)
        else:
            self.adamw_step_fn = adamw_step_fused
            self.muon_step_fn = muon_step_fused
    ...
    def _step_muon(self, group):
        ...
        self.muon_step_fn(
            stacked_grads, stacked_params, state["momentum_buffer"], state["second_momentum_buffer"],
...
TOTAL_BATCH_SIZE = 2**16  # 65K tokens per step (Mac-tuned)
EMBEDDING_LR = 0.6
...
DEPTH = 4  # Mac-tuned (4 layers)
DEVICE_BATCH_SIZE = 16  # Mac-tuned (fits 32GB)
...
def build_model_config(depth):
    ...
    return GPTConfig(
        sequence_len=MAX_SEQ_LEN, vocab_size=vocab_size, n_layer=depth, n_head=num_heads, n_kv_head=num_heads, n_embd=model_dim, window_pattern=WINDOW_PATTERN,
    )
...
while True:
    ...
    print(
        f"\rstep {step:05d} ({pct_done:.1f}%) | loss: {debiased_smooth_loss:.6f} | lrm: {lrm:.2f} | dt: {dt*1000:.0f}ms | tok/sec: {tok_per_sec:,} | epoch: {epoch} | remaining: {remaining:.0f}s    ",
        end="",
        flush=True,
    )
